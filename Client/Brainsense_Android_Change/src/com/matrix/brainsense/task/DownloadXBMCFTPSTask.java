...
public class DownloadXBMCFTPSTask extends AsyncTask<String, Integer, String> {
	...
	public DownloadXBMCFTPSTask(Context context, ProgressBar progressBar) {
	...
	protected String doInBackground(String... params) {
		...
		try {
			if (FTPSUtil.connect(PathUtil.ftpIp, PathUtil.ftpPort,
				...
				currentSize = FTPSUtil.getLocalFolderSize(new File(
						localXBMCPath));
				totalSize = FTPSUtil.getFolderSize(downloadXBMCPath);
				if (totalSize == 0) {
					return "1";
				}
				if (!thread_flag) {
				...
				FTPSUtil.downloadFile(downloadXBMCPath, localXBMCPath, false);
			} else {
				return "1";
			}
	...
	protected void onPostExecute(String result) {
		...
		if (result != null) {
			...
			// The FTP didn't encrypt with SSL
			if (result.equals("2")) {
	...
	private void checkWifi() {
		...
		if (!flag) {
			...
			new Handler().postDelayed(new Runnable() {
				@Override
				public void run() {
					new DownloadXBMCFTPSTask(context, progressBar)
							.execute(downloadXBMCPath);
				}
			}, 4000);
		}
	...
	class myThread extends Thread {
		...
		public void run() {
			while (true) {
				int progress = (int) ((FTPSUtil.currentSize * 1.0 / totalSize) * 10000);
